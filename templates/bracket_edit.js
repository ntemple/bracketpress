//code for removal of value from checkbox of round 3 and so on.
...
//        if (prnt != 'match15' && prnt != 'match30' && prnt != 'match45' && prnt != 'match60' && prnt != 'match61' && prnt != 'match62' && prnt != 'match63') {

            var d = '';
            /*
             Process to fill up next dropdowns
             */
            //change_next_dds_on_change_current_round_dd($(this));
            ...
            for (var m = 0; m < num_more_rounds; m++) {
                ...
                if (next_round_div) {
                    ...
                    //check if next round has this current value in it
            ...
            /*
             Process to fill up previous dropdowns
             */
            if (current_selected_index > 0) {
                for (var n = 0; n < num_prev_rounds - 1; n++) {
                    ...
                    if (prev_round_div_id) {
                        ...
                        $('#' + prev_round_div_id + ' select').each(function (i, value) {//alert('here'+i);
                            var t = $(this);
                            var opts = t.children('option');
                            for (var l = 0; l < opts.length; l++) {
                                if (opts[l].value == current_selected_value) {
                                    //t.children('option').eq(3).attr('selected','selected');
                                    //alert(opts[l].value);
                                    t.val(opts[l].value);
                                    $.ajax({
                                        type: 'POST',
...
//        }
        ...
        /**
         * NLT 2013/01/26
         * Functionality specific to the final four. This has been
         * brute-forced with internal knowledge of the structure of the brackets
         * Needs to be refactored into a more elegant system.
         *
         * This is completely brute force. Any change could affect the final 4,
         * this really only needs to be run on specific changes.
         * For now, we rebuild on every update.
         * @todo: optimize javascript for final 4
         */
        ...
        // Add the items
        set_team(61, 1, $('#get_team_selector_south_match15').val(), winner );
        ...
        // Add the items
        set_team(62, 1, $('#get_team_selector_east_match45').val(), winner );
        set_team(62, 2, $('#get_team_selector_midwest_match60').val(), winner );
